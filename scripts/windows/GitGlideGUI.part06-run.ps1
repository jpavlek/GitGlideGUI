...
# Initialize and show
Set-HelpExamples
Append-Log -Text 'Git Glide GUI - Enhanced Version ready.' -Color ([System.Drawing.Color]::DarkGreen)
...
Write-AuditLog -Message ("STARTUP | RepoRoot='{0}' | Version=v$($script:GitGlideGuiVersion)" -f $script:RepoRoot)
...
Apply-UiMode
Set-CommandPreview -Title ('Welcome to Git Glide GUI v{0}' -f $script:GitGlideGuiVersion) -Commands 'Hover a button to preview its commands.' -Notes 'Use Setup for repository setup, UI mode, command palette, and GitHub publish/diagnostics guidance. Simple mode keeps everyday actions visible, Workflow mode shows Git Flow steps, and Expert mode shows every tool. Use Integrate for Merge & Publish workflows, Recovery for the State Doctor/conflicts, and History / Graph for branch inspection. Press ESC to cancel running operations.'
